...
def processGAMEatAllMoves(fname,representation='raw',history=0,gs=5):
    moves =readGAME(fname)
    boards = []
    labels = []
    
    board = numpy.zeros((gs, gs))
    n = 0
    for move in moves:
        if n%2 == 0:
            boards.append(board.copy())
        ...
        labels.append(move[0]*gs + move[1])
        playMove(move,board,n,representation=representation)
        n+=1
    
    histbds = [([numpy.zeros((gs,gs))]*(history-i) + boards[:i+1] if i<history else boards[i-history:i+1]) for (i,bd) in enumerate(boards)]
    return(histbds,labels) 
    
def processDistGAMEatAllMoves(fname,representation='raw',history=0,gs=5):
    (moves,dists) =readDistGAME(fname)
    boards = []
    labels = dists
...
def processGAMEs(fnames,representation='raw',history=0,gs=5):
    bls = map(lambda fname: processGAMEatAllMoves(fname,representation,history,gs),fnames)
    (boards,labels) = map(lambda game: reduce(lambda a,b: a+b,game),zip(*bls))
    return (boards,labels)  
    
def processDistGAMEs(fnames,representation='raw',history=0,gs=5):
    bls = map(lambda fname: processDistGAMEatAllMoves(fname,representation,history,gs),fnames)
    (boards,labels) = map(lambda game: reduce(lambda a,b: a+b,game),zip(*bls))
    return (boards,labels)  
