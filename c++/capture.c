...
int playout(int *moves,int nMoves)
{
    ...
    for (int i = 0; i<nMoves; i++){
        ...
        playMove(board,libs,moves[i],r,player,&nLegal,legal,neighs,sz);
        player = 3-player;
    }
...
if (x%(ng/20)==0) printf(" GAME:: %d\n",x);
    ...
    for (int i = 0; i<sz*sz*2; ++i){
        ...
        if (res>0){
            ...
            break;
            //return res;   // TODO: replace for real playouts
        }
        if (res==-2){
            debug =1;
            displayAll(board,libs,legal,&nLegal,sz);
...
int playMove(int *board,int *libs,int move, int r, int colour, int *nLegal, int *legal,int *neighs, int sz){
    ...
    // check if legal   FIXME: look case on the left for move: 1
    if (l==0){
        int cons = 0;
        for (int i=0;i<n;i++){
